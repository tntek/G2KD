...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            ...
            netB.eval()
            netC.eval()
            mem_label_soft, mem_outputs_new, mtx_infor_nh, feas_FC = obtain_label(dset_loaders['test'], netF, netB, netC, args, iter_num, iter_sw)
            mem_label_soft = torch.from_numpy(mem_label_soft).cuda()
            feas_all = feas_FC[0]
            ops_all = feas_FC[1]
            feas_all_B = feas_FC[2]
            netF.train()
            netB.train()
            netC.train()
        ...
        # features_F_mix = 0.9*features_F_self + 0.1*features_F_nh
        # outputs_test_mix = netC(netB(features_F_mix))
        outputs_test_mix = netC(features_F_self_B)
        # outputs_test_self = netC(netB(features_F_self)) 

        # --------------------------Loss_nc--------------------------------------------------------------------------------
        feat_t = F.normalize(features_F_self_B.cuda())
        feat_mat = torch.mm(feat_t, feas_all_B.cuda().t()) / 0.05
        ...
        # feat_new = torch.cat([feat_mat, feat_mat2], 1).cuda()
        feat_new = F.softmax(feat_mat2, 1) 
        loss_entropy =loss.entropy_loss().cuda()
        nc_loss = 0.1 * loss_entropy(feat_new).cuda()
        ...
        if args.cls_par > 0:
            ...
            classifier_loss = classifier_loss + KD_loss

            classifier_loss = classifier_loss + nc_loss
        ...
        if args.ent:
            ...
            if args.gent:
                ...
                entropy_loss -= 0.8 * gentropy_loss
...
def obtain_label(loader, netF, netB, netC, args, num_iter_f, iter_sw_f):
    ...
    outputs_new_norm = outputs_new_soft.cpu().numpy()
    ...
    print(log_str + '\n')
    return pred_label_re.astype('int'), outputs_new_norm, mtx_re, feas_re
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='./ckpskdr0/target_gskd_ts_smf/')
    parser.add_argument('--output_src', type=str, default='./ckpskdr0/source/')
    ...
    print("mix------mix---target_GKD_final_nc_C_noshortmf----KD----")
